...
function logSequelizeError(err: any) {
  // Compatible con distintos tipos de errores de Sequelize
  const parent = err?.parent || err?.original || {};
  console.error('SEQUELIZE NAME:', err?.name);
  console.error('PG CODE:', parent?.code);
  console.error('PG DETAIL:', parent?.detail);
  console.error('PG MESSAGE:', parent?.message || err?.message);
}
